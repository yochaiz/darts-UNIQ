from torch import load as loadModel, nn
...
class TinyNet(nn.Module):
    def __init__(self, crit, bitwidths, kernel_sizes):
    ...
    def nLayers(self):
        return sum(1 for m in self.modules() if isinstance(m, Conv2d) or isinstance(m, nn.Linear))
    ...
    def switch_stage(self, k):
        pass

    # load original pre_trained model of UNIQ
    def loadUNIQPre_trained(self, path, logger, gpu):
        map = {}
        map['features.0'] = 'features.0.ops.0.op.0'
        map['features.1'] = 'features.0.ops.0.op.1'

        map['features.3'] = 'features.2.ops.0.op.0'
        map['features.4'] = 'features.2.ops.0.op.1'

        map['features.6'] = 'features.4.ops.0.op.0'
        map['features.7'] = 'features.4.ops.0.op.1'

        map['features.9'] = 'features.6.ops.0.op.0'
        map['features.10'] = 'features.6.ops.0.op.1'
